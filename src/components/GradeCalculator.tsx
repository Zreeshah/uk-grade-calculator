...
export default function GradeCalculator({
  ...
  return (
    <div className="card">
      {title && <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6">{title}</h2>}

      {/* Result display */}
      {average !== null && result && (
        <div className="mb-6 p-5 rounded-xl bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/50 dark:to-purple-950/50 border border-indigo-100 dark:border-indigo-800">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Your Overall Grade</p>
              <p className="text-3xl font-bold text-slate-900 dark:text-white">{average.toFixed(2)}%</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Classification</p>
              <p className={`text-xl font-bold ${result.classColor}`}>{result.classification}</p>
            </div>
            {type === "gpa" && gpa !== null && (
              <div className="text-right">
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">GPA Equivalent</p>
                <p className="text-xl font-bold text-indigo-600 dark:text-indigo-400">{gpa.toFixed(1)}</p>
              </div>
            )}
          </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-700">
              <th className="text-left text-sm font-medium text-slate-500 dark:text-slate-400 pb-3 pr-3">
                {type === "btec" ? "Unit Name" : "Module Name"}
              </th>
              <th className="text-left text-sm font-medium text-slate-500 dark:text-slate-400 pb-3 pr-3 w-28">
                Grade (%)
              </th>
              {showCredits && (
                <th className="text-left text-sm font-medium text-slate-500 dark:text-slate-400 pb-3 pr-3 w-24">
                  Credits
                </th>
              )}
          ...
          {targetEnabled && (
            <div className="mt-4 flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <label className="block text-sm text-slate-500 dark:text-slate-400 mb-1.5">Target Average (%)</label>
                <input
                  type="number"
                  min="0"
                  max="100"
              ...
              <div className="flex-1">
                <label className="block text-sm text-slate-500 dark:text-slate-400 mb-1.5">Remaining Credits</label>
                <input
                  type="number"
                  min="1"
